...
class Discriminator(nn.Module):
    def __init__(self, input_dim = 64 ** 2 * 3, output_dim = 1000):
        super(Discriminator, self).__init__()
...
class Generator(nn.Module):
    def __init__(self, z_dim, output_dim = 64 ** 2 * 3):
...
'''
# TODO conv2d arch for d and g
class Discriminator(nn.Module):
    def __init__(self, nc = 3, ndf = 64, output_units = 1000):
        super(Discriminator, self).__init__()
        print("init Discrim w/ nc=", nc, ", ndf=", ndf)
        self.ndf = ndf
...
class Generator(nn.Module):
    def __init__(self, z_dim, ngf = 64, output_dim = 64 ** 2 * 3):
        super(Generator, self).__init__()
        self.z_dim = z_dim
        self.main = nn.Sequential(
    ...
    def forward(self, batch_size, cuda = False):
        if not self.training:
            with torch.no_grad():
                x = Variable(torch.rand(batch_size, self.z_dim, 1, 1), requires_grad = False)
        else:
            x = Variable(torch.rand(batch_size, self.z_dim, 1, 1), requires_grad = False)
