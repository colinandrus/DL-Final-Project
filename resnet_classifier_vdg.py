...
data_dir = r'D:\nyu-gsas\ds1008-deeplearning\ssl_data_96\supervised'
model_save_path = r'resnet_reconstructionMASK2.pth'
model_load_path = r'resnet_reconstructionMASK.pth'
...
use_gpu = True
epochs = 100
num_samples_per_class = int(sys.argv[3])
...
def get_subsample_indices(img_dataset_to_sample, samples_per_class):
    if samples_per_class == 64:
        return img_dataset_to_sample
    sample_indices = []
    for i in range(0, samples_per_class):
        sample_indices = sample_indices + [ j*64 + i for j in range(0,1000) ]
    sample_indices.sort()
    return torch.utils.data.Subset(img_dataset_to_sample, sample_indices)



sampled_img_datasets = {x: get_subsample_indices(image_datasets[x], num_samples_per_class) for x in ['train', 'val']}

dataloaders = {x: torch.utils.data.DataLoader(sampled_img_datasets[x], batch_size=batch_size,
